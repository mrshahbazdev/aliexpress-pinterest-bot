...
.nav{background:#12121f;padding:14px 24px;display:flex;align-items:center;
     justify-content:space-between;border-bottom:1px solid #1e1e35;position:sticky;top:0;z-index:50}
.nav h1{font-size:18px;color:#ff4757;letter-spacing:.5px}
.nav-links{display:flex;gap:6px}
.nav-links a{color:#888;text-decoration:none;font-size:13px;padding:6px 14px;border-radius:6px;
             transition:.2s}
...
.stat{background:#12121f;border-radius:10px;padding:18px;text-align:center;border:1px solid #1e1e35}
.stat .n{font-size:28px;font-weight:700;color:#ff4757}
.stat .l{font-size:12px;color:#666;margin-top:2px}

/* --- Card / Section --- */
.card{background:#12121f;border-radius:12px;padding:22px;border:1px solid #1e1e35;margin-bottom:20px}
.card h2{color:#ff4757;font-size:16px;margin-bottom:14px;display:flex;align-items:center;gap:8px}

/* --- Alerts --- */
.alert{padding:10px 16px;border-radius:8px;margin-bottom:14px;font-size:13px}
.alert-ok{background:rgba(22,199,132,.12);color:#16c784;border:1px solid #16c78466}
.alert-err{background:rgba(234,57,67,.12);color:#ea3943;border:1px solid #ea394366}
.alert-info{background:rgba(255,71,87,.08);color:#ff4757;border:1px solid #ff475766}

/* --- Buttons --- */
.btn{display:inline-flex;align-items:center;gap:6px;padding:8px 18px;border-radius:7px;
     border:none;cursor:pointer;font-size:13px;font-weight:500;text-decoration:none;transition:.15s}
.btn-red{background:#ff4757;color:#fff}.btn-red:hover{background:#e03e4d}
.btn-green{background:#16c784;color:#fff}.btn-green:hover{background:#12a86d}
.btn-gray{background:#222238;color:#aaa;border:1px solid #333}.btn-gray:hover{background:#2a2a45}
.btn-sm{padding:5px 12px;font-size:12px}

/* --- Form --- */
.form-row{display:flex;gap:10px;align-items:end;flex-wrap:wrap}
.fg{display:flex;flex-direction:column;gap:3px}
.fg label{font-size:11px;color:#666;text-transform:uppercase;letter-spacing:.5px}
.fg input,.fg select,.fg textarea{background:#0a0a0f;border:1px solid #333;color:#e0e0e0;
    border-radius:6px;padding:8px 12px;font-size:13px;font-family:inherit}
.fg textarea{min-height:80px;resize:vertical;font-family:'Courier New',monospace;font-size:12px}

/* --- Product grid --- */
.pgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:14px}
.pcard{background:#0e0e1a;border:1px solid #1e1e35;border-radius:10px;overflow:hidden;
...
.pcard .ttl{font-size:13px;font-weight:600;margin-bottom:6px;display:-webkit-box;
...
.pcard .disc{background:#ff4757;color:#fff;padding:1px 7px;border-radius:4px;font-size:11px;font-weight:700}
.pcard .meta{display:flex;gap:10px;font-size:11px;color:#666;margin-bottom:10px}
...
/* --- Pin content box --- */
.pin-box{background:#0a0a0f;border-radius:8px;padding:10px 12px;margin-top:10px;
         border:1px solid #1a1a30;font-size:12px}
...
.pagi a{padding:6px 13px;background:#12121f;color:#888;border-radius:6px;text-decoration:none;
...
.cookie-status{display:inline-flex;align-items:center;gap:6px;font-size:12px;padding:4px 10px;
               border-radius:20px}
...
<nav class="nav">
  ...
  <div class="nav-links">
    <a href="/" class="{{ 'active' if page_name == 'dashboard' }}">Dashboard</a>
    <a href="/settings" class="{{ 'active' if page_name == 'settings' }}">Settings</a>
    <a href="/fetch" class="{{ 'active' if page_name == 'fetch' }}">Fetch Products</a>
    <a href="/products" class="{{ 'active' if page_name == 'products' }}">Saved ({{ saved_count }})</a>
  </div>
</nav>
<div class="container">
{% if msg %}<div class="alert alert-{{ msg_cls }}">{{ msg }}</div>{% endif %}
...
{% block content %}
<div class="stats">
  <div class="stat"><div class="n">{{ stats.total }}</div><div class="l">Total Products</div></div>
  <div class="stat"><div class="n">{{ stats.with_pins }}</div><div class="l">Pinterest Ready</div></div>
  <div class="stat"><div class="n">{{ stats.without_pins }}</div><div class="l">Pending AI</div></div>
</div>
<div class="card">
  <h2>Quick Actions</h2>
  <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
    ...
    {% else %}
      <span class="cookie-status none">No cookies &mdash; <a href="/settings" style="color:#ea3943">set up first</a></span>
    {% endif %}
    ...
    <form method="post" action="/generate-all" style="display:inline">
      <button class="btn btn-green">Generate All Pinterest Content ({{ stats.without_pins }})</button>
    </form>
    {% endif %}
  </div>
</div>
...
<div class="card">
  ...
  <p style="color:#666;font-size:13px;margin-bottom:14px">
    Paste the full <code>cookie:</code> header from your browser DevTools (Network tab)
    when logged into <a href="https://portals.aliexpress.com" target="_blank" style="color:#ff4757">portals.aliexpress.com</a>.
    The code will extract all needed cookies automatically.
  </p>
  <form method="post" action="/settings">
    <div class="fg" style="margin-bottom:14px">
      <label>Full Cookie Header</label>
      <textarea name="raw_cookie" rows="6" placeholder="cna=...; xman_us_t=...; xman_us_f=...">{{ raw_cookie }}</textarea>
    </div>
    <div class="form-row" style="margin-bottom:14px">
      <div class="fg">
        <label>Tracking ID</label>
        <input name="tracking_id" value="{{ tracking_id }}" placeholder="default">
      </div>
      <div class="fg">
        <label>Ship To</label>
        <input name="ship_to" value="{{ ship_to }}" placeholder="US" style="width:80px">
      </div>
      <div class="fg">
        <label>Currency</label>
        <input name="currency" value="{{ currency }}" placeholder="USD" style="width:80px">
      </div>
      <div class="fg">
        <label>Language</label>
        <input name="language" value="{{ language }}" placeholder="en" style="width:80px">
      </div>
    </div>
  ...
  <form method="post" action="/settings/ai">
    <div class="form-row" style="margin-bottom:14px">
      <div class="fg" style="flex:1">
        ...
        <input name="gemini_key" type="password" value="{{ gemini_key }}" placeholder="AIza...">
      </div>
      <div class="fg" style="flex:1">
        <label>OpenAI API Key (alternative)</label>
        <input name="openai_key" type="password" value="{{ openai_key }}" placeholder="sk-...">
      </div>
    </div>
...
<div class="card">
  ...
  {% if not cookie_ok %}
  <div class="alert alert-err">No cookies configured. <a href="/settings" style="color:#ea3943">Go to Settings</a> first.</div>
  {% endif %}
  <form method="post" action="/fetch">
    <div class="form-row">
      <div class="fg">
        <label>Page</label>
        <input type="number" name="page" value="{{ page_num }}" min="1" max="999" style="width:80px">
      </div>
      <div class="fg">
        <label>Count</label>
        <select name="count">
          <option value="6" {{ 'selected' if count == 6 }}>6</option>
          <option value="12" {{ 'selected' if count == 12 }}>12</option>
        </select>
      </div>
      <button type="submit" class="btn btn-red" {{ 'disabled' if not cookie_ok }}>Fetch &amp; Preview</button>
    </div>
  </form>
</div>
...
<div class="card">
  ...
  <div style="margin-bottom:14px;display:flex;gap:8px;flex-wrap:wrap">
    <form method="post" action="/save-page" style="display:inline">
      ...
      <input type="hidden" name="products_json" value="{{ products_json }}">
      <button type="submit" name="action" value="save" class="btn btn-green">Save All to DB</button>
      <button type="submit" name="action" value="save_gen" class="btn btn-red">Save + Generate Pinterest</button>
    </form>
  </div>
  <div class="pgrid">
  {% for p in products %}
    <div class="pcard">
      ...
      <div class="body">
        ...
        <div class="acts">
          <form method="post" action="/save-single" style="display:inline">
            <input type="hidden" name="product_json" value='{{ p | tojson }}'>
            <button type="submit" class="btn btn-green btn-sm">Save</button>
          </form>
          <a href="{{ p.item_url }}" target="_blank" class="btn btn-gray btn-sm">AliExpress</a>
          {% if p.is_duplicate %}<span class="badge badge-dup">Already Saved</span>{% endif %}
        </div>
      </div>
    </div>
  {% endfor %}
...
<div class="pagi">
  {% if page_num > 1 %}<a href="/fetch?page={{ page_num - 1 }}&count={{ count }}">&larr; Prev</a>{% endif %}
  <a class="cur">Page {{ page_num }}</a>
  <a href="/fetch?page={{ page_num + 1 }}&count={{ count }}">Next &rarr;</a>
</div>
{% endif %}
{% endblock %}
"""
...
<div class="card">
  ...
  <div style="margin-bottom:14px;display:flex;gap:8px">
    {% if pending_count > 0 %}
    <form method="post" action="/generate-all" style="display:inline">
      <button class="btn btn-green">Generate Pinterest for {{ pending_count }} pending</button>
    </form>
    {% endif %}
  </div>
  <div class="pgrid">
    ...
    <div class="pcard">
      ...
      <div class="body">
        ...
        <div class="meta">
          <span>Sales: {{ p.sales_30day }}</span>
          <span>Rating: {{ p.comment_score }}</span>
          {% if p.pin_generated %}<span class="badge badge-ok">Pinterest Ready</span>
          {% else %}<span class="badge badge-warn">No Pin</span>{% endif %}
        </div>
        {% if p.pin_generated %}
        <div class="pin-box">
        ...
        <div class="acts" style="margin-top:10px">
          ...
          <form method="post" action="/generate/{{ p.item_id }}" style="display:inline">
            <button class="btn btn-green btn-sm">Generate Pinterest</button>
          </form>
          {% endif %}
          {% if p.promo_url %}
          <a href="{{ p.promo_url }}" target="_blank" class="btn btn-gray btn-sm">Promo Link</a>
          {% endif %}
          <a href="{{ p.item_url }}" target="_blank" class="btn btn-gray btn-sm">AliExpress</a>
          <form method="post" action="/delete/{{ p.item_id }}" style="display:inline"
                onsubmit="return confirm('Delete?')">
            <button class="btn btn-sm" style="background:#2a1520;color:#ea3943">Delete</button>
          </form>
        </div>
      </div>
    </div>
  ...
  <div class="pagi">
    ...
    <a href="/products?page={{ pg }}" class="{{ 'cur' if pg == current_page }}">{{ pg }}</a>
    {% endfor %}
  </div>
  {% endif %}
  {% else %}
...
def _get_ai_keys() -> tuple[str, str]:
    """Return (gemini_key, openai_key) from DB or config."""
    db = get_db()
    config = get_config()
    gemini = db.get_setting("gemini_key") or config.gemini_api_key
    openai = db.get_setting("openai_key") or config.openai_api_key
    return gemini, openai


def _generate_pin_for_product(product: Product) -> bool:
    """Generate Pinterest content and save to DB. Returns True on success."""
    db = get_db()
...
def dashboard() -> str:
    ...
    return _render("dashboard", page_name="dashboard", stats=stats, cookie_ok=cookie_ok)
...
def settings_page() -> str:
    ...
    return _render(
        ...
        gemini_key=db.get_setting("gemini_key") or get_config().gemini_api_key,
        openai_key=db.get_setting("openai_key") or get_config().openai_api_key,
    )
...
def fetch_products() -> str:
    ...
    ae = _get_ae_client()
    if not ae:
        return redirect(url_for("settings_page", msg="Set cookies first!", msg_cls="err"))
    ...
    msg = (
        f"Found {len(products)} products on page {page_num}"
        if products
        else "No products found. Cookies may be expired."
    )
...
def generate_single(item_id: str) -> str:
    ...
    try:
        ...
        if ok:
            msg = f"Pinterest content generated for: {db_product.title[:40]}..."
            msg_cls = "ok"
        else:
